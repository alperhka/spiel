...
-- (3) In Windows das Verzeichnis "C:/Zimmermann/volumes/postgres/tablespace/spiel" anlegen
--     Siehe .extras\compose\postgres\compose.yml
-- (4) 2. PowerShell:
--     cd .extras\compose\backend\postgres
--     docker compose exec db bash
--        chown postgres:postgres /var/lib/postgresql/tablespace
--        chown postgres:postgres /var/lib/postgresql/tablespace/spiel
...
-- (7) 2. PowerShell:
--     docker compose exec db bash
--        psql --dbname=postgres --username=postgres --file=/sql/create-db-spiel.sql
--        psql --dbname=spiel --username=spiel --file=/sql/create-schema-spiel.sql
--        exit
...
CREATE ROLE spiel LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE spiel;

GRANT ALL ON DATABASE spiel TO spiel;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE spielspace OWNER spiel LOCATION '/var/lib/postgresql/tablespace/spiel';
