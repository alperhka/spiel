    ...
    stages {
        ...
        stage('Init') {
            ...
            steps {
                ...
                sh 'rm -rf .extras/doc/projekthandspiel/html'

                // https://www.jenkins.io/doc/pipeline/steps/git
                // "named arguments" statt Funktionsaufruf mit Klammern
                git url: 'https://github.com/juergenzimmermann/spiel', branch: 'main', poll: true
            }
        }
        ...
        stage('Install') {
            steps {
                ...
                // /var/jenkins_home/workspace/spiel (siehe "pwd" oben)
                sh 'cat package.json'
        ...
        stage('Test, Codeanalyse, Security, Dok.') {
            ...
            post {
                always {
                    ...
                    publishHTML (target : [
                        reportDir: '.extras/doc/projekthandspiel/html',
                        reportFiles: 'projekthandspiel.html',
                        reportName: 'Projekthandspiel',
                        reportTitles: 'Projekthandspiel'
                    ])
                ...
                success {
                    script {
                        if (fileExists("${env.WORKSPACE}/spiel.zip")) {
                            sh 'rm spiel.zip'
                        }
                    }
                    // https://www.jenkins.io/doc/pipeline/steps/pipeline-utility-steps/#zip-create-zip-file
                    zip zipFile: 'spiel.zip', archive: false, dir: 'dist'
                    // jobs/spiel/builds/.../archive/spiel.zip
                    archiveArtifacts 'spiel.zip'
                }
            }
        }
        ...
        stage('Docker Image bauen') {
            steps {
                ...
                // def image = docker.build("juergenzimmermann/spiel:${env.BUILD_ID}")
                // image.push()
                // image.push('latest')
            }
        }
