...
> ❗ Vor dem 1. Start von PostgreSQL muss man die Skripte `create-db-spiel.sql`
> und `create-schema-spiel.sql` aus dem Verzeichnis `.extras\db\postgres\sql`
...
> nach `C:\Zimmermann\volumes\postgres\csv\spiel`.

> ❗ Vor dem 1. Start von MySQL muss man das Skript `create-db-spiel.sql` aus
...
> dem Verzeichnis `.extras\db\mysql\csv` nach `C:\Zimmermann\volumes\mysql\csv\spiel`.
...
`db.populate` in `src\config\resources\spiel.yml` wird festgelegt, ob die
(Test-) DB `spiel` neu geladen wird.
...
- Im Eingabefeld _Name_ `spiel` und im Eingabefeld _Summary_ z.B.
...
und wählt aus dem Verzeichnis `.extras\postman` die Datei `spiel.postman_environment.json`
aus. Jetzt hat man die Umgebung `spiel` mit der Variablen `base_url` und dem
Wert `https://localhost:3000` angelegt.

Im Environment `spiel` muss man die Variable `client_secret` auf den Wert setzen,
der in Keycloak beim _Realm acme_ in _Clients > spiel-client > Credentials_
...
```powershell
    npm exec jest --detectOpenHandles --errorOnDeprecated `
      --forceExit --runTestsByPath '__tests__\spiel\spiel-GET.controller.test.ts'
```
...
```powershell
    ...
    docker build --tag juergenzimmermann/spiel:2025.4.1-bookworm .

    # Alpine
    Get-Content Dockerfile.alpine | docker run --rm --interactive hadolint/hadolint:2.12.1-beta-debian
    docker build --tag juergenzimmermann/spiel:2025.4.1-alpine --file Dockerfile.alpine .
```
...
```powershell
    docker history juergenzimmermann/spiel:2025.4.1-bookworm
    docker history juergenzimmermann/spiel:2025.4.1-alpine
```
...
```powershell
    docker inspect juergenzimmermann/spiel:2025.4.1-bookworm
    docker inspect juergenzimmermann/spiel:2025.4.1-alpine
```
...
```powershell
    docker sbom juergenzimmermann/spiel:2025.4.1-bookworm
    docker sbom juergenzimmermann/spiel:2025.4.1-alpine
```
...
```powershell
    cd .extras\compose\spiel

    # PowerShell fuer spiel-Server mit Bookworm-Image zzgl. DB-Server und Mailserver
    docker compose up

    # Nur zur Fehlersuche: weitere PowerShell für bash
    cd .extras\compose\spiel
    docker compose exec spiel bash
        id
        env
        exit
    ...
    # 2. Powershell: spiel-Server einschl. DB-Server und Mailserver herunterfahren
    cd .extras\compose\spiel
    docker compose down
```
...
```powershell
    docker scout quickview juergenzimmermann/spiel:2025.4.1-bookworm
    docker scout quickview juergenzimmermann/spiel:2025.4.1-alpine
```
...
```powershell
    docker scout cves juergenzimmermann/spiel:2025.4.1-bookworm
    docker scout cves --format only-packages juergenzimmermann/spiel:2025.4.1-bookworm
````
