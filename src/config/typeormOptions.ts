...
import { Spiel } from '../spiel/entity/spiel.entity.js';
import { entities } from '../spiel/entity/entities.js';
...
const database = (db?.name as string | undefined) ?? Spiel.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Spiel.name.toLowerCase();
